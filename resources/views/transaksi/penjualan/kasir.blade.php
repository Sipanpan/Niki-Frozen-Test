...
<div class="space-y-4">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="relative">
                <div class="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none">
                    <svg class="w-5 h-5 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                </div>
                <input type="text" id="search-produk" placeholder="Cari atau scan barcode produk..."
                    class="w-full h-14 pl-12 pr-4 text-sm font-medium bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl focus:ring-2 focus:ring-blue-500/20 outline-none transition-all dark:text-white dark:placeholder-slate-500" autofocus>
            </div>

            {{-- Category Filter --}}
            <div class="flex gap-2 overflow-x-auto pb-2 custom-scrollbar">
                <button class="flex-shrink-0 px-4 py-2 rounded-xl text-xs font-bold bg-blue-500 text-white shadow-sm">Semua</button>
                @foreach($kategoris ?? [] as $kat)
                    <button class="flex-shrink-0 px-4 py-2 rounded-xl text-xs font-bold bg-white dark:bg-white/5 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-white/10 hover:bg-blue-50 dark:hover:bg-blue-500/10 transition-colors">{{ $kat->nama }}</button>
                @endforeach
            </div>
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3" id="product-grid">
                @forelse($produks ?? [] as $produk)
                    <button onclick="addToCart({{ $produk->id }}, '{{ $produk->nama }}', {{ $produk->harga_jual }})"
                        ...
                        <div class="w-full aspect-square rounded-xl bg-slate-100 dark:bg-white/5 mb-2 flex items-center justify-center overflow-hidden">
                            @if($produk->gambar)
                                <img src="{{ asset('storage/' . $produk->gambar) }}" alt="{{ $produk->nama }}" class="w-full h-full object-cover">
                            @else
                                <svg class="w-8 h-8 text-slate-300 dark:text-slate-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"></path></svg>
                            @endif
                        </div>
                        <p class="text-xs font-bold text-slate-800 dark:text-white truncate">{{ $produk->nama }}</p>
                        <p class="text-xs font-black text-blue-500 mt-0.5">Rp {{ number_format($produk->harga_jual, 0, ',', '.') }}</p>
        ...
        {{-- Cart Panel --}}
        <div class="bg-white dark:bg-white/5 rounded-2xl border border-slate-100 dark:border-white/10 shadow-sm flex flex-col h-fit lg:sticky lg:top-4">
            ...
            <div class="p-4 border-t border-slate-100 dark:border-white/10 space-y-3">
                ...
                <div class="grid grid-cols-2 gap-2">
                    <button onclick="processPayment('tunai')" class="h-12 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white rounded-xl font-bold text-sm hover:shadow-lg transition-all active:scale-[0.98]">
                        💵 Tunai
                    </button>
                    <button onclick="processPayment('qris')" class="h-12 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-xl font-bold text-sm hover:shadow-lg transition-all active:scale-[0.98]">
                        📱 QRIS
                    </button>
                </div>
            </div>
...
function processPayment(method) {
    ...
    alert('Pembayaran ' + method.toUpperCase() + ' — Fitur akan diintegrasikan dengan backend');
}
</script>
@endpush
@endsection
