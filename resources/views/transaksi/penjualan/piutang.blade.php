            ...
            <table class="w-full text-sm">
                ...
                <tbody>
                    ...
                    <tr class="border-b border-slate-50 dark:border-white/5 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                        <td class="p-4 font-mono font-bold text-blue-500">{{ $pi->penjualan->no_nota ?? '-' }}</td>
                        <td class="p-4 text-slate-600 dark:text-slate-300">{{ $pi->penjualan->pelanggan->nama ?? 'Umum' }}</td>
                        <td class="p-4 text-right font-bold text-slate-800 dark:text-white">Rp {{ number_format($pi->jumlah, 0, ',', '.') }}</td>
                        <td class="p-4 text-right font-bold text-rose-500">Rp {{ number_format($pi->sisa, 0, ',', '.') }}</td>
                        <td class="p-4 text-center text-slate-600 dark:text-slate-300">{{ $pi->jatuh_tempo?->format('d/m/Y') ?? '-' }}</td>
                        <td class="p-4 text-center"><span class="px-2 py-1 rounded-lg text-xs font-bold {{ $pi->status == 'lunas' ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400' : 'bg-rose-50 text-rose-600 dark:bg-rose-500/10 dark:text-rose-400' }}">{{ ucfirst(str_replace('_', ' ', $pi->status)) }}</span></td>
                    </tr>
                    @empty
                    <tr><td colspan="6" class="p-8 text-center text-slate-400">Tidak ada piutang</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
